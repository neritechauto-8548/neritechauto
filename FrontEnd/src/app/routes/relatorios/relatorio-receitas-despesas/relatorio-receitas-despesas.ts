...
export class RelatorioReceitasDespesas implements OnInit {
  ...
  isSumCategory(cat: string): boolean {
    return cat === 'Total de receitas' || cat === 'Total de despesas' || cat === 'Resultado líquido';
  }
  ...
  private processData(realizados: any[], pagar: any[], receber: any[]) {
    ...
    // 3. Calculate Totals (Total Receitas, Total Despesas, Resultado Líquido)
    for (const colKey of this.columnsList) {
      // Total Receitas
      ...
      // Resultado líquido (No rollover balance!)
      this.dataMatrix['Resultado líquido'][colKey].realizado =
        this.dataMatrix['Total de receitas'][colKey].realizado -
        this.dataMatrix['Total de despesas'][colKey].realizado;

      this.dataMatrix['Resultado líquido'][colKey].previsto =
        this.dataMatrix['Total de receitas'][colKey].previsto -
        this.dataMatrix['Total de despesas'][colKey].previsto;
    }
  ...
  private generateChartData() {
    ...
    const seriesResultRealizado: number[] = [];
    const seriesResultPrevisto: number[] = [];
    ...
    for (let i = 0; i < colCount; i++) {
      ...
      const resR = this.dataMatrix['Resultado líquido'][colKey].realizado;
      const resP = this.dataMatrix['Resultado líquido'][colKey].previsto;
      ...
      seriesResultRealizado.push(resR);
      seriesResultPrevisto.push(resP);
    }
    ...
    if (this.mode === 'PREVISTO E REALIZADO' || this.mode === 'Somente realizado') {
      ...
      seriesData.push({ name: 'Resultado Realizado', type: 'line', data: seriesResultRealizado });
    ...
    if (this.mode === 'PREVISTO E REALIZADO' || this.mode === 'Somente previsto') {
      ...
      seriesData.push({ name: 'Resultado Previsto', type: 'line', data: seriesResultPrevisto });
    ...
    this.totals = {
      ...
      resultado: this.columnsList.reduce((acc, c) => acc + this.dataMatrix['Resultado líquido'][c][targetType], 0)
    };
  }
  ...
  exportExcel() {
    let csvContent = '\uFEFF';
    csvContent += `Relatório Receitas x Despesas - ${this.period} (${this.year})\r\n`;
    ...
    const filename = `receitas_despesas_${this.period.toLowerCase()}_${this.year}.csv`;
