...
function is(reqInfo: RequestInfo, path: string) {
  ...
  // Suporta chamadas com e sem prefixo de caminho (ex.: '/api') em dev
  const base = environment.baseUrl ?? '';
  const isHttpBase = /^https?:\/\//i.test(base);
  const isPathPrefix = !isHttpBase && base.startsWith('/');
  const normalizedBase = isPathPrefix ? base.replace(/\/$/, '') : '';

  const patterns: RegExp[] = [];
  // Sem prefixo
  patterns.push(new RegExp(`^/?${path}(/)?$`, 'i'));
  // Com prefixo de caminho (ex.: '/api/user', '/api/auth/login')
  if (normalizedBase) {
    patterns.push(new RegExp(`^${normalizedBase}/${path}(/)?$`, 'i'));
  }
